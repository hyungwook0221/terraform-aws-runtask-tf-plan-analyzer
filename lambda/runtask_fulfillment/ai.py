# 2025.03.25 18:20 잘 동작하는 것 확인된 코드
import json
import os
import re
...
def eval(tf_plan_json):
    ...
    prompt = """
        Terraform plan 내 AMI 변경 사항을 다음 규칙에 따라 분석해 주세요:
        1. Amazon Machine Image (AMI 또는 image_id)가 변경된 경우, 기존과 새로운 AMI의 Linux kernel, Docker, ECS agent 정보를 비교해 주세요.
        2. 분석 내용 안에 "AMI: ami-xxxxxxx" 형식으로 표현된 ID가 있으면 이를 직접 추출하여 비교에 사용하세요.
        3. 추출한 AMI ID는 get_ami_releases 함수를 통해 세부 정보를 조회할 수 있습니다.
        4. 사고 과정을 <thinking> 태그 내에 단계적으로 작성해 주세요.
        5. 아래 결과 형식을 사용해 주세요. 서론은 생략해 주세요.
        6. 결과는 반드시 한국어로 작성해 주세요.
        <output>
        <thinking>
        </thinking>
        <result>
        ...
        for content in response["content"]:
            if "toolUse" in content:
                ...
                if tool["name"] == "GetECSAmisReleases":
                    ...
                    # us-east-1 리전으로 EC2 클라이언트 생성 후 AMI 조회
                    ec2_client = boto3.client("ec2", region_name="us-east-1")
                    try:
                        response = ec2_client.describe_images(ImageIds=tool["input"]["image_ids"])
                        release_details_info = response["Images"]
                    except Exception as e:
                        logger.error(f"AMI 조회 실패: {str(e)}")
                        release_details_info = "No release notes were found the ami."    
    ...
    prompt = f"""
        ...
        - 결과는 한글로 출력하고 번호를 붙히는 대신 '##' 및 '-'을 사용해 가시성을 높게 출력해주세요.
